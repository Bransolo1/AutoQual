...
export class StudiesService {
  ...
  async create(input: CreateStudyInput) {
    return this.prisma.study.create({
      data: {
        ...
        screeningLogic: input.screeningLogic ?? undefined,
        interviewGuide: input.interviewGuide ?? undefined,
        syntheticEnabled: input.syntheticEnabled ?? false,
        localizationChecklist: input.localizationChecklist ?? undefined,
        recruitmentChecklist: input.recruitmentChecklist ?? undefined,
        activationChecklist: input.activationChecklist ?? undefined,
        rolloutPlan: input.rolloutPlan ?? undefined,
        distributionTracking: input.distributionTracking ?? undefined,
        deliveryHealth: input.deliveryHealth ?? undefined,
      }
    });
  }
  ...
  async updateLocalizationChecklist(studyId: string, checklist: Record<string, boolean>) {
    return this.prisma.study.update({
      where: { id: studyId },
      data: { localizationChecklist: checklist },
    });
  }
  ...
  async updateRecruitmentChecklist(studyId: string, checklist: Record<string, boolean>) {
    return this.prisma.study.update({
      where: { id: studyId },
      data: { recruitmentChecklist: checklist },
    });
  }
  ...
  async updateActivationChecklist(studyId: string, checklist: Record<string, boolean>) {
    return this.prisma.study.update({
      where: { id: studyId },
      data: { activationChecklist: checklist },
    });
  }
